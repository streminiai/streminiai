...
class ChatOverlayService : Service(), View.OnTouchListener {
    ...
    private var menuItems: List<ImageView> = emptyList()
    ...
    private fun setupOverlay() {
        ...
        menuItems.getOrNull(0)?.setOnClickListener {
            if (!isAnimating) {
                collapseMenu()
                handleRefresh()
            }
        }
        menuItems.getOrNull(1)?.setOnClickListener {
            if (!isAnimating) {
                collapseMenu()
                handleSettings()
            }
        }
        menuItems.getOrNull(2)?.setOnClickListener {
            if (!isAnimating) {
                collapseMenu()
                handleAIChat()
            }
        }
        menuItems.getOrNull(3)?.setOnClickListener {
            if (!isAnimating) {
                collapseMenu()
                handleScanner()
            }
        }
        menuItems.getOrNull(4)?.setOnClickListener {
            if (!isAnimating) {
                collapseMenu()
                handleVoiceCommand()
            }
